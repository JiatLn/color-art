...
pub(crate) const XN: f64 = 0.95047;
pub(crate) const YN: f64 = 1.0;
pub(crate) const ZN: f64 = 1.08883;
...
pub(crate) fn xyz_lab(t: f64) -> f64 {
    if t > T3 { t.powf(1.0 / 3.0) } else { t / T2 + T0 }
}

pub(crate) fn rgb_xyz(r: f64) -> f64 {
    if r <= 0.04045 { r / 12.92 } else { ((r + 0.055) / 1.055).powf(2.4) }
}
...
pub(crate) fn xyz_rgb(r: f64) -> f64 {
    if r <= 0.00304 { 12.92 * r } else { 1.055 * r.powf(1.0 / 2.4) - 0.055 }
}
