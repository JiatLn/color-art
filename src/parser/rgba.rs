...
pub fn parser_rgba_str(s: &str) -> Result<(f64, f64, f64, f64)> {
    let s = s
        .trim()
        .to_lowercase()
        .replace(" ", "")
    ...
    if a < 0.0 || a > 1.0 {
        return Err(anyhow::anyhow!("alpha must be between 0 and 1, got {}", a));
    }
...
mod tests {
    ...
    fn test_parser_rgba() {
        ...
        let (r, g, b, a) = parser_rgba_str(s).unwrap();
